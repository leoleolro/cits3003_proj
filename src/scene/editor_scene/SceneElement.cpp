...
void EditorScene::LitMaterialComponent::add_material_imgui_edit_section(MasterRenderScene& /*render_scene*/, const SceneContext& /*scene_context*/) {
    ...
    material_changed |= ImGui::DragFloat("Diffuse Factor",&material.diffuse_tint[3],0.01f,0.0f,50.0f);
    material_changed |= ImGui::ColorEdit3("Specular Tint", &material.specular_tint[0]);
    material_changed |= ImGui::DragFloat("Specular Factor", &material.specular_tint[3],0.01f,0.0f,50.0f);
    material_changed |= ImGui::ColorEdit3("Ambient Tint", &material.ambient_tint[0]);
    material_changed |= ImGui::DragFloat("Ambient Factor", &material.ambient_tint[3],0.01f,0.0f,50.0f);
    material_changed |= ImGui::DragFloat("Shininess", &material.shininess, 1.0f, 0.0f,138.0f);
    material_changed |= ImGui::DragFloat("Texture Scale", &material.texture_scaling, 0.01f);
...
void EditorScene::LitMaterialComponent::update_material_from_json(const json& json) {
    ...
    //TASK C
    material.texture_scaling = m["texture_scaling"];
}
...
json EditorScene::LitMaterialComponent::material_into_json() const {
    return {"material", {
        ...
        //TASK C
        {"texture_scaling", material.texture_scaling},
    }};
}
